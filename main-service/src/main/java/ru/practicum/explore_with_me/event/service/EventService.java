...
import ru.practicum.explore_with_me.client.EndpointHit;
...
import ru.practicum.explore_with_me.event.model.EventSort;
import ru.practicum.explore_with_me.event.model.EventState;
import ru.practicum.explore_with_me.event.model.UpdateEventRequest;
...
public class EventService implements PageTool {
    ...
    public EventFullDto publishEventByAdmin(Long eventId) {
        Event eventDb = eventRepository.findById(eventId).orElseThrow(
                () -> new ObjectNotFoundException("Event not found id=" + eventId));
        if (eventDb.getState().equals(EventState.PENDING)) {
    ...
    public EventFullDto rejectEventByAdmin(long eventId) {
        Event eventDb = eventRepository.findById(eventId).orElseThrow(
                () -> new ObjectNotFoundException("Event not found id=" + eventId));
        if (!eventDb.getState().equals(EventState.PUBLISHED)) {
    ...
    public EventFullDto getInfoAboutEventByUserId(long userId, long eventId) throws ObjectNotFoundException {
        Event eventDb = eventRepository.findById(eventId).orElseThrow(
    ...
    public EventFullDto cancelEvent(long userId, long eventId) throws ObjectNotFoundException {
        Event eventDb = eventRepository.findById(eventId).orElseThrow(
                () -> new ObjectNotFoundException("Event not found"));
        if (userId != eventDb.getInitiator().getId()) {
            throw new InputDataException("User with id = " + userId + " not create event with id = " + eventId);
